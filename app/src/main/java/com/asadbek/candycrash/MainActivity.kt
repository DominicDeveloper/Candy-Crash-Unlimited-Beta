...
import com.asadbek.candycrash.uiltel.OnSwipeListener
import java.util.Arrays.asList

class MainActivity : AppCompatActivity() {
    // first we create transparent drawable
    ...
    private fun candyInterChacge() {
        val background :Int= candy.get(candyToBeReplaced).tag as Int
        val background1 :Int= candy.get(candyToBeDragged).tag as Int
    ...
    private fun checkRowForThree(){
        for (i in 0..61){
            ...
            if (!list.contains(i)){
                ...
                if (candy.get(x++).tag as Int == chosedCandy && !isBlank && candy.get(x++).tag as Int == chosedCandy && candy.get(x).tag as Int == chosedCandy){
                    score = score + 3
